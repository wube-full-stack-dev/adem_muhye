...
const conn = require("../conf/db.cofig"); // Fixed: changed from db.cofig to db.config
...
async function install() {
  ...
  const sqlFile = __dirname + "/initial_sql.sql"; // Fixed: removed fs.readFileSync from here
  console.log(sqlFile); // This will print the path
  ...
  // Read the sql file - FIXED: changed queryfile to sqlFile
  ...
  for (let i = 0; i < queries.length; i++) {
    try {
      ...
      console.log("Err Occurred - Table not created", err.message);
